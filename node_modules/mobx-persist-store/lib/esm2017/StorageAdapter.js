...
class StorageAdapter {
    ...
    async setItem(key, item) {
        ...
        const data = this.options.expireIn
            ? Object.assign({}, item, {
                __mps__: {
                    expireInTimestamp: (0, utils_1.buildExpireTimestamp)(this.options.expireIn),
                },
            })
            : item;
        const content = stringify ? JSON.stringify(data) : data;
        (0, utils_1.consoleDebug)(debugMode, `${key} - setItem:`, content);
    ...
    async getItem(key) {
        ...
        const hasExpired = (0, utils_1.hasTimestampExpired)((_b = parsedData.__mps__) === null || _b === void 0 ? void 0 : _b.expireInTimestamp);
        (0, utils_1.consoleDebug)(debugMode, `${key} - hasExpired`, hasExpired);
        if (hasExpired && removeOnExpiration) {
            await this.removeItem(key);
        }
        parsedData = hasExpired ? {} : parsedData;
        (0, utils_1.consoleDebug)(debugMode, `${key} - (getItem):`, parsedData);
        return parsedData;
    }
    async removeItem(key) {
        var _a;
        const { debugMode = false } = this.options;
        (0, utils_1.consoleDebug)(debugMode, `${key} - (removeItem): storage was removed`);
